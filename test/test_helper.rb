...
Rails::Generator::Base.prepend_sources(Rails::Generator::PathSource.new(:clearance, "#{File.dirname(__FILE__)}/../generators/"))
...
module ActiveRecord
  ...
  class Base
    class << self
      attr_accessor :pluralize_table_names, :timestamped_migrations, :connection
    end
...
require 'initializer'
    ...
    cp      "#{RAILS_ROOT}/../permanent/config/database.yml", "#{RAILS_ROOT}/config/database.yml"
    mkdir_p "#{RAILS_ROOT}/db"
    mkdir_p "#{RAILS_ROOT}/test/fixtures"
    mkdir_p "#{RAILS_ROOT}/public/stylesheets"
    cp      "#{RAILS_ROOT}/../permanent/Rakefile", "#{RAILS_ROOT}/Rakefile"    
  ...
  def teardown# Mocks out the configuration
